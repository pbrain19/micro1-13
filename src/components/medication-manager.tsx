...
export default function MedicationManager({ medications, onAdd, onUpdate, onDelete }: MedicationManagerProps) {
  const [isAddModalOpen, setIsAddModalOpen] = useState(false)
  const [isEditModalOpen, setIsEditModalOpen] = useState(false)
  const [isDeleteModalOpen, setIsDeleteModalOpen] = useState(false)
  const [currentMedication, setCurrentMedication] = useState<MedicationSchedule | null>(null)
  ...
  const handleToggleComplete = (medication: MedicationSchedule) => {
    ...
    toast.success(`Medication marked as ${medication.isComplete ? "incomplete" : "complete"}`)
  }
  ...
  return (
    <Card>
      ...
      <CardContent>
        {medications.length === 0 ? (
          <div className="text-center py-6">
            <p className="text-muted-foreground">No medications scheduled. Add one to get started.</p>
          </div>
        ) : (
              ...
              .map((medication) => (
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <h3
                        className={`font-medium ${medication.isComplete ? "line-through text-muted-foreground" : ""}`}
                      >
                        {medication.title}
                      </h3>
                      <p className="text-sm text-muted-foreground">{medication.medicationName}</p>
      ...
      {/* Add Modal */}
      <AddEditMedicationModal open={isAddModalOpen} onOpenChange={setIsAddModalOpen} onSave={handleAdd} />
